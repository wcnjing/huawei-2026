"""Soft chibi 24x32 pixel sprites: rounded cheeks and compact bodies."""
...
def draw_character(a, x, y, scale=1, index=0):
    ...
    def r(xx, yy, w, h, c):
        # Remove four body rows; keep the facial pixel grid untouched.
        def compact(row):
            return row - sum(row > cut for cut in (20,24,29,32))
        top, bottom = compact(yy), compact(yy+h)
        a.rect(x+xx*scale,y+top*scale,w*scale,(bottom-top)*scale,c)
    ...
    r(6,10,12,3,ss); r(7,12,10,2,ss); r(9,14,6,1,ss)
    r(7,5,10,7,skin); r(6,8,1,3,skin)
    r(7,10,10,3,skin); r(8,13,8,1,skin); r(10,14,4,1,skin)
    r(7,7,2,3,sh); r(9,6,6,2,sh); r(9,12,3,1,sh)
    r(16,7,1,5,ss); r(15,12,1,1,ss)
    r(5,9,1,2,skin); r(18,9,1,2,ss)
    # Small bright eyes, soft blush, and a tiny upward smile.
    for ex in (8,14):
        r(ex,8,2,3,'#342d3d')
        r(ex,8,1,1,'#fff5dc')
        r(ex+1,10,1,1,'#66575c')
    blush = '#db8e80' if index!=2 else '#c7806f'
    r(6,11,3,1,blush); r(15,11,3,1,blush)
    mouth = '#ad7768' if index!=2 else '#8d554b'
    r(10,12,1,1,mouth); r(13,12,1,1,mouth); r(11,13,2,1,mouth)
...
def draw_studies(art_class):
    a=art_class(196,44,'Cute compact human pixel characters with bright eyes, rosy cheeks and tiny smiles')
    a.palette={}
    a.rect(0,0,196,44,'#29283f')
    for i,p in enumerate(PALETTES):
        x=4+i*32
        a.rect(x+4,37,18,2,'#1b1c31')
        draw_character(a,x,6,index=i)
        a.rect(x+9,42,8,1,p['cloth'][2])
        sprite=art_class(24,32,p['name'].replace('-',' ')+' cute pixel character')
        draw_character(sprite,0,0,index=i)
        sprite.save('character-v4-'+p['name'],scale=6)
    a.save('neon-character-studies-v4',scale=3)
